...
ch_CoCodeAssignment_Utilized_AllStates_Public_txt \
    = os.path.isfile(FilePath + 'CoCodeAssignment_Utilized_AllStates_Public.txt')
ch_CoCodeAssignment_Utilized_AllStates_Public_zip \
    = os.path.isfile(FilePath + 'CoCodeAssignment_Utilized_AllStates_Public.zip')
...
for file in files_to_remove:
    ...
    if os.path.exists(file_path):
        try:
            ...
            pass
            #print(f"PermissionError: Could not delete {file_path} because it is being used by another process.")
...
open(FilePath + 'CoCodeAssignment_Utilized_AllStates_Public.zip', 'wb').write(r.content)
        ...
        if len(npanxx) == 7:
            ...
            if country == 'CANADA':
                ...
                if state_name == 'ONTARIO':
                    mapfile = 'on-s.png'
                return render_template("canada-ac-oc.html", LinkHome=LinkHome, ac_return=ac_return,
                                       companyname=companyname,
                                       state_name=state_name, state=state, country=country, acoc_return=acoc_return,
    ...
    if request.args['sort'] == 'company':
        ...
        try:
            ...
            while row:
                ocnall_list.append(row)
                row = cur.fetchone()
            return render_template("ca-ocn-all.html", LinkHome=LinkHome, header=header, ocnall_list=ocnall_list,
    ...
    if request.args['sort'] == 'npa':
        ...
        try:
            ...
            while row:
                ocnall_list.append(row)
                row = cur.fetchone()
            return render_template("ca-ex-all.html", LinkHome=LinkHome, header=header, ocnall_list=ocnall_list,
                                   caoc_headers=caoc_headers)
        except:
            return render_template("error.html", LinkHome=LinkHome, error="Unable to find OCN")
    if request.args['sort'] == 'company':
...
def areacodes():
    ...
    if 'npa' in request.args and len(request.args['npa']) == 3:
        ...
        if country == 'CANADA':
            ...
            for item in statuslist:
                cur.execute(
                    "select COCodeNXX from caoc where NPA='" + npa_code + "'" + "and Status='" + item + "' order by COCodeNXX;")
                row = cur.fetchall()
                for items in row:
